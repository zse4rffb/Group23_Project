...
def print_menu(exits, room_items, inv_items, room_objects, room_characters):
    ...
    print("Inspect items for valuable information.")
...
def execute_go(direction):
    ...
    if direction in current_room["exits"]:
        if move(current_room["exits"], direction) == rooms["N/407"] and stairs == 0:
            ...
            time.sleep(5)
            print("you ascend to the second floor, Half way there!")
            time.sleep(5)
...
def execute_speak(char_id):
    ...
    if found:
        print("")
        if (char_id == "guard") and (item_chocolate in inventory) and guard_talk == 0:
            print("""
            .-----------------------------------------------------------------------.
            ...
            """)
            print("")
            answer = normalise_input(input("Y/N: "))
            if answer == "y" or "yes":
                print("")
                print("""
                 .-----------------------------------------------------------------------.
                ...
                guard_talk = 1
        elif (char_id == "guard") and guard_talk == 1:
            print("""
             .-----------------------------------------------------------------------.
...
def execute_use1(use_id):
    global current_room
    if (use_id in objects) and (objects[use_id] in current_room["objects"]):
        if use_id == "passwordlock":
    ...
    RECEPTION."""
            ...
            print("")
            print(objects[use_id]["use"]["null"])
    else:
        print("")
        print("you can't use that.")


def execute_use2(use_id):
    global inventory
    global current_room
    if (use_id[0] in items and inventory) and (use_id[1] in objects) and (objects[use_id[1]] in current_room["objects"]):
        if use_id[0] == "torch" and use_id[1] == "pinata":
        ...
        elif use_id[0] == "keys" and use_id[1] == "door":
    ...
    head. Out of the corner of your eye you spot a piece of paper."""
        ...
        elif use_id[0] in objects[use_id[1]]["use"]:
...
def execute_open(open_id):
    if (open_id in objects) and (objects[open_id] in current_room["objects"]):
        if open_id == "presents":
        ...
        elif open_id == "door" and item_keys in inventory:
            inventory.remove(item_keys)
            current_room["name"] = "N/4.07"
            current_room["description"] = """
    Kirills body lies lifeless on the floor in the centre of the room, there 
...
def win():
    ...
    for word in murderer_list:
        ...
        x = 1
    if normalised_murderer == "elon musk" or normalised_murderer == "elon" or normalised_murderer == "musk": 
        murderer_correct = True
        weapon = str(input("""
        And what weapon do you think they used?: """))
        normalised_weapon = normalise_input(weapon)
        if "manual" in normalised_weapon:
	        weapon_correct = True
        else:
            print("""
        ...
        weapon_correct = False
        guess += 1
    if murderer_correct == True and weapon_correct == True:
        game_ending = True
        end()
    elif guess == 3:
        print("""
